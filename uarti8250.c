...
enum {					/* registers */
	...
	Msr		= 6,		/* Modem Status */
	Scr		= 7,		/* Scratch Pad */
//	Mdr		= 8,		/* Mode Def'n (not on mt7688) */
//	Usr		= 31,		/* Uart Status Register; missing in mt7688? */
...
{	.regs	= &i8250ctlr[0], /* not [2] */
	.name	= "uartlite",
	.freq	= 40000000,	/* Not used, we use the global i8250freq */
	.phys	= &i8250physuart,
	.console= 1,
	.baud = 115200,
	.next	= nil, },
...
//	csr8o(ctlr, Mdr, (mode & ~Modemask) | Modeuart);
	...
	if(ie){
		if(ctlr->iena == 0 && !ctlr->poll){
			intrenable(IRQuartl, i8250interrupt, uart, 0, "uartl");
			ctlr->iena = 1;
		}
